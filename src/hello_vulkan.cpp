...
void HelloVulkan::setup(const VkInstance& instance, const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t queueFamily)
{
  ...
  m_offscreen.setup(device, physicalDevice, &m_alloc, queueFamily, defaultSearchPaths);
  m_raytrace.setup(device, physicalDevice, &m_alloc, queueFamily, defaultSearchPaths);
}
