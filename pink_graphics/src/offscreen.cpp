...
void Offscreen::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice, nvvk::ResourceAllocator* allocator, uint32_t queueFamily, std::vector<std::string> defaultSearchPaths)
{
...
void Offscreen::createPipeline(VkRenderPass& renderPass)
{
  ...
  pipelineGenerator.addShader(nvh::loadFile("spv/passthrough.vert.spv", true, m_defaultSearchPaths, true), VK_SHADER_STAGE_VERTEX_BIT);
  pipelineGenerator.addShader(nvh::loadFile("spv/post.frag.spv", true, m_defaultSearchPaths, true), VK_SHADER_STAGE_FRAGMENT_BIT);
